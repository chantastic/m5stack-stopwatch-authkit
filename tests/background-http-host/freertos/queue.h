...
inline int xQueueSend(FakeQueue *q,void *pointer,uint32_t wait){std::unique_lock<std::mutex> hold(q->lock);if(wait==portMAX_DELAY)q->ready.wait(hold,[&]{return !q->value||FakeRtos::shutdown;});if(FakeRtos::shutdown)throw FakeRtos::Stop{};if(q->value)return 0;std::memcpy(&q->value,pointer,sizeof(void*));q->ready.notify_all();return 1;}
